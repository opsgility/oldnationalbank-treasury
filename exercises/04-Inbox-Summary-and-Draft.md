# Exercise 4 — Inbox Summary to Treasury Briefing & Draft as Delegate

- **App:** Outlook on the web
- **Workflow:** Catch up on Treasury mail, summarize threads into a briefing, and draft replies and commentary
- **Material:** the four practice threads from the [Practice Inbox Pack](../assets/Practice-Inbox-Pack.md)
- **Estimated time:** 20 minutes
- **Difficulty:** Core — the daily content work

---

## Objective

Use Copilot in Outlook to do the writing-heavy part of Treasury fast and well: catch up across the inbox, turn a multi-message thread into a crisp briefing, and draft replies and a short liquidity note in your own professional voice. Summarizing and drafting were two of the top tasks this cohort flagged — this is where Copilot saves the most time.

## Before you start

- [ ] The four practice threads are in your inbox (sent to yourself in pre-work).
- [ ] Outlook on the web open, Copilot pane available, and the **Summarize** button visible at the top of a thread.

## What Copilot is doing here

Copilot summarizes and drafts over the mail you can open. **It does not send anything** — every draft is a first pass you finish, and on anything that commits the firm (a covenant certificate, a revolver decision, a note to the banks) you own the words and the numbers. Verify amounts, dates, rates, and commitments before anything goes out.

---

## Block A — Catch up & summarize

### Step 1 — Catch me up (Copilot pane)
```text
Catch me up on my Treasury email from the last few days. Group it by topic, and for each item give a one-line status, any deadline, the dollar figures mentioned, and who owns the next step. Flag anything that needs a decision from me.
```

### Step 2 — Thread to briefing
Open the **Weekly Liquidity Review — Week of 6/8** thread and click **Summarize**, then refine:
```text
Summarize this thread as a 5-bullet briefing for my manager: the cash position vs. target, the BofA sweep question and its resolution, the idle CAD decision, and the Week 9 forecast dip with the two options on the table. End with a single line: "Decision needed from you."
```

### Step 3 — Pull the action items
```text
From this thread, list only the open action items as a checklist: who owns each, what they committed to, and the due date. Leave out anything already resolved.
```

---

## Block B — Draft & reply

### Step 4 — Reply in your voice
Open the **Bank Fee Analysis — Q2 Account Analysis Statements** thread:
```text
Draft a reply from me (Treasury Operations) confirming I'll consolidate the three banks' account analysis statements into one comparison workbook, calling out that I'll show per-unit pricing by bank, identify the top fee categories driving the 9% increase, and flag anything we pay for but don't use. Professional and concise, three short paragraphs. Do not send it — I'll review.
```
Read it aloud. If it doesn't sound like you, tell Copilot what to change (shorter, warmer, more direct) — don't accept a draft that isn't your voice.

### Step 5 — Draft a sensitive note carefully
Open the **Debt Covenant Certificate — Q2 Compliance** thread:
```text
Draft a short cover note to the agent bank transmitting our Q2 covenant compliance certificate. State that we are in compliance with the minimum liquidity, fixed charge coverage, and total leverage covenants, that the supporting schedule is attached, and ask them to confirm receipt. Leave the actual ratio values as [FCCR], [Leverage], and [Liquidity] placeholders for me to fill in and verify. Formal tone.
```
> **Why placeholders:** covenant figures are exactly what Copilot can get subtly wrong, and it's the firm's name on the certificate. You fill and verify every number.

### Step 6 — Turn an email into a standalone note
```text
Using the FX Exposure — CAD Repatriation thread, draft a short internal note to Corporate Tax summarizing the proposed move (keep the CAD in a CAD sweep, no conversion at the current ~0.73 rate) and asking them to confirm there are no intercompany or tax issues before we execute. Keep it to one short paragraph.
```

---

## Deliverable

A topic-grouped inbox catch-up, a 5-bullet liquidity briefing with action items, and three drafted-but-unsent messages (fee-analysis reply, covenant cover note with placeholders, Tax note) — each reviewed in your voice.

## Verify before you rely on it

- [ ] Every dollar figure, rate, ratio, and date in a draft traces back to the thread — Copilot did not invent a number.
- [ ] The covenant cover note still has **placeholders**, not Copilot-guessed ratios.
- [ ] Each draft reads in your voice and commits only to what you actually intend.
- [ ] Nothing was sent automatically — you click send.

## Key takeaways

- Summarizing a 3+ message thread is the highest-frequency, lowest-risk win — master it first.
- The structure you request is the quality you get — ask for bullets, owners, deadlines, and dollar figures.
- On anything that commits the firm, **draft with placeholders** and verify every number yourself.

---

*Next: Exercise 5 — Cash & Liquidity Update Deck.*
